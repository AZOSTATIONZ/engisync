...
export async function createWorkspace(
  ...
  redirect(`/dashboard/projects/${workspace.id}`);
}
...
export async function joinWorkspace(
  ...
  if (workspace.requireApproval) {
    ...
    await createNotification({
      ...
      link: `/dashboard/projects/${workspace.id}`,
    });
...
export async function approveJoinRequest(requestId: string): Promise<ActionState> {
  ...
  await createNotification({
    ...
    link: `/dashboard/projects/${req.workspaceId}`,
  });

  revalidatePath(`/dashboard/projects/${req.workspaceId}`);
...
export async function inviteByEmail(
  ...
  const url = `${await getBaseUrl()}/dashboard/projects/invite/${token}`;
...
export async function revokeInvite(inviteId: string): Promise<ActionState> {
  ...
  revalidatePath(`/dashboard/projects/${invite.workspaceId}`);
...
export async function joinViaInvite(token: string): Promise<ActionState> {
  ...
  if (already) redirect(`/dashboard/projects/${invite.workspaceId}`);
  ...
  revalidatePath("/dashboard/projects");
  redirect(`/dashboard/projects/${invite.workspaceId}`);
}
