...
import { Crown, KeyRound, ShieldCheck, ClipboardCheck, AlertCircle } from "lucide-react";
...
} from "@/app/dashboard/collaboration-ui";
import {
  Card,
  CardContent,
  CardDescription,
...
import { projectTasks, projectTeam } from "@/lib/routes";
import {
  AccessSettingsForm,
  JoinRequestRow,
  InviteManager,
...
export default async function WorkspaceDetailPage({
  ...
  return (
    <div className="space-y-8">
      <div>
        {/* Name, department, stage and pace come from the project layout. This
            page owns people: who is here, what they may do, and how someone
            joins or leaves. */}
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div />
          {isLeader ? (
            <span className="flex items-center gap-1 rounded-full bg-primary/10 px-3 py-1 text-sm text-primary">
              <Crown className="h-4 w-4" /> Project leader
            </span>
          ) : (
            <RequestWithdrawal
        ...
        <div className="flex items-start gap-3 rounded-lg border border-amber-500/40 bg-amber-500/10 p-4">
          ...
          <div className="text-sm">
            ...
            <p className="text-muted-foreground">
              ...
              participation.{" "}
              <Link href={projectTasks(workspace.id)} className="font-medium text-primary hover:underline">
                Go to tasks
              </Link>
              .
            </p>
      ...
      {/* Tasks, documents, discussions and the rest are tabs in the project
          shell now, so this page no longer carries its own copy of them.
          Only the leader's approval queue is specific to managing people. */}
      {isLeader && (
        <div>
          <Button asChild size="sm">
            <Link href={`${projectTeam(workspace.id)}/manage`}>
              <ClipboardCheck className="h-4 w-4" /> Approval queues
            </Link>
          </Button>
        </div>
      )}
